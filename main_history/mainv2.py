...
# Build model
def build_model(input_shape, filters=64, kernel_size=(3, 3)):
    ...
    model = keras.Sequential()
    model.add(ConvLSTM2D(filters=filters, kernel_size=(7, 7), input_shape=input_shape, padding='same',activation=LeakyReLU(alpha=0.01), return_sequences=True))
    model.add(BatchNormalization())
    model.add(ConvLSTM2D(filters=filters, kernel_size=(5, 5), padding='same',activation=LeakyReLU(alpha=0.01), return_sequences=True))
    model.add(BatchNormalization())
...
# Build the model
model = build_model(input_shape, filters=32, kernel_size=(3, 3))
model.summary()
